...
switch(state) {
    ...
    case STATE_EXPECT_NAME:
        ...
        // read the length
        // NOTE: read the joining player's information (name, color, armor types)
        var len, temp;
        len = read_ubyte(player.socket);
        player.name = read_string(player.socket, len);
        player.name = string_copy(player.name, 0, MAX_PLAYERNAME_LENGTH);
        player.name = string_replace_all(player.name, "#", " ");
        player.bodyColor = read_ubyte(player.socket);
        player.helmetColor = read_ubyte(player.socket);
        player.bodyType = read_ubyte(player.socket);
        player.helmetType = read_ubyte(player.socket);
        player.shoulderType = read_ubyte(player.socket);
        player.accessoryType = read_ubyte(player.socket);
        
        ds_list_add(global.players, player);
        ServerPlayerJoin(player, global.sendBuffer);
